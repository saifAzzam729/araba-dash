...
export default function ({accountId}) {
    ...
    const {isError, isLoading, refetch} = useQuery(
        ["eBay-policy", currentPage, searchTerm, preferredTableContentLocale],
        () =>
    ...
    const {mutate: toggleMutation, isLoading: isToggleLoading} = useMutation(
        (data) => EBayService.update(data.id, {publish: data.publish, featured: data.featured}),
        {
            onSuccess: () => {
                refetch();
                showSuccessAlert({});
    ...
    const openAddPage = (item) => {
        navigate(makeLocaleUrl(`/eBay-policies/add/${accountId}`));
    }
    const viewPage = (item) => {
        navigate(makeLocaleUrl(`/ebay-return-policy/${accountId}/view/${item.id}`));
    }
