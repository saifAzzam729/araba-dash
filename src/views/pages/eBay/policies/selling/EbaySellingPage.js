...
export default function ({accountId}) {
    ...
    const {isError, isLoading, refetch} = useQuery(
        ["eBay-selling-policy", currentPage, searchTerm, preferredTableContentLocale],
        () =>
    ...
    const openAddPage = (item) => {
        navigate(makeLocaleUrl(`/ebay-selling-policy/add/${accountId}`));
    }
    const viewPage = (item) => {
        navigate(makeLocaleUrl(`/ebay-selling-policy/${accountId}/view/${item.id}`));
    }
    if (isError) {
