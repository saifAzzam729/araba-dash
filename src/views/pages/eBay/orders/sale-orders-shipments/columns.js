...
export const ManifestIcon = ({row, manifestMutation, disabled}) => {
    const isClickable = !disabled && (row?.status !== "MANIFEST" && row?.labelUrl !== 'NULL');

    const handleManifestClick = () => {
        if (isClickable) {
            const toMutateObject = {
                SaleOrderShipments: [row.id],
            };
            manifestMutation(toMutateObject);
        }
    };
    ...
    return (
        <Button
            disabled={disabled}
            color={'link'}
            className={`mx-1 ${isClickable ? "cursor-pointer text-info" : "text-muted"}`}
...
const DownloadLabelColumn = CreateColumn({
    ...
    maxWidth: '100px',
    cellCustomizationFunction: (row) => <DownloadLabelIcon labelUrl={row.labelUrl}
...
export const createColumns = (manifestMutation) => {
    ...
    const ManifestColumn = CreateColumn({
        ...
        maxWidth: '100px',
        cellCustomizationFunction: (row) => (
            <ManifestIcon row={row} manifestMutation={manifestMutation}
                          disabled={row?.status === 'VALID' || row?.status === 'UN_COMPLETED'}/>
        )
    });
