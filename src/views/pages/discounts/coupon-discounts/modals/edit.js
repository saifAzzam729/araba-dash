...
function EditCouponDiscountModal({isOpen, closeModal, onEditSuccessCb, item = null}) {
    ...
    const schema = yup.object().shape({
        ...
        couponCode: yup.string().required(translate('forms.field-required')),
        usageLimitPerUser: yup.number().required(translate('forms.field-required')),
        ...
        applicableTo : yup.object().required("This field is Required"),
    ...
    useQuery(
        ...
        () => DiscountsService.getById(item.id),
        {
            onSuccess: (res) => {
                ...
                setValue("applyToAll", data.applyToAll);


                const valueType = data.type === 'FIXED' ? 'Number' : data.type;
                ...
                setValue("applicableTo", {
                    value: data.applicableTo,
                    label: data.applicableTo,
                });
                const categoryValue = data.categories.map((category) => {
                    return {
                        value: category.id, label:category.name
                    }
                })
                setValue("categories", categoryValue);
                ...
                setValue("usageLimitPerUser", data.usageLimitPerUser);

                const userGroupsValue = data.userGroup.map((user) => {
                    return {
                        value: user.id, label: user.name
                    }
                })
                setValue("userGroups", userGroupsValue);

                const TagsValue = data.tags.map((tag) => {
                    return {
    ...
    const {data} = useQuery(
        ...
        () => OptionsService.getApplicableToOptions({
    ...
    const {mutate, isError, isLoading} = useMutation(
        (data) => DiscountsService.update(item.id, data),
        {
    ...
    const promiseTagsOptions = (
        ...
        const perPage = 10;
        const search = params.search;
        TagsService.getPagination({page: page,
            search: search ,
            limit: perPage,
            locale: preferredTableContentLocale
        }).then((res) => {
    ...
    return (
        <CustomModal translatedHeader={translate("discount.common.edit-coupon-discount")} isOpen={isOpen} closeModal={closeModal}>
                ...
                <div className="d-flex flex-column" style={{gap: "1rem"}}>
                    ...
                    <div className="row">
                        <div className="col-6">
                            <CustomControlledInputField
                                name={'couponCode'}
                                label={translate("discount.forms.couponCode")}
                                control={control}
                                errors={errors}
                            />
                        </div>
                        <div className="col-6">
                            <CustomControlledInputField
                                ...
                                label={translate("discount.forms.usageLimitPerUser")}
                                type={'number'}
                                control={control}
                                errors={errors}
                            />
...
export default EditCouponDiscountModal