...
import AddShipmentProductModal from "./modals/add";
import EditShipmentProductsModal from "./modals/edit";
import ViewShipmentProductModal from "./modals/view";
...
export default function () {
	...
	const { isError, isLoading, refetch } = useQuery(
		["shipment-products", currentPage, searchTerm, preferredTableContentLocale],
		() =>
	...
	const { mutate: deleteMutation } = useMutation(
		(data) => ShipmentProductsService.deleteObject(data.id),
		{
			onSuccess: () => {
				refetch();
				showSuccessAlert({});
	...
	if (isError) {
		return <ErrorPage title={"shipment-products"} />;
	}

	const permissionObject = createPermissionObjectForUi(
		PERMISSIONS_NAMES.ROLE_SHIPMENT_PRODUCT_ADD,
		PERMISSIONS_NAMES.ROLE_SHIPMENT_PRODUCT_UPDATE,
		PERMISSIONS_NAMES.ROLE_SHIPMENT_PRODUCT_DELETE,
		PERMISSIONS_NAMES.ROLE_SHIPMENT_PRODUCT_SHOW,
		PERMISSIONS_NAMES.ROLE_SHIPMENT_PRODUCT_LIST
	);
	...
	return (
		<>
			<BreadCrumbs title={"shipment-products"} data={[]} />
			<TableBase
				columns={COLUMNS}
				data={items}
				onPaginate={(page) => {
