...
function AddAffiliatesModal({isOpen, closeModal, onAddSuccessCb, item = null}) {
    ...
    const {mutate, isError, isLoading} = useMutation(
        (data) => AffiliatesService.create(data),
        {
    ...
    useEffect(() => {
        if(slugValue && !slugValue.startsWith(SLUG_PREFIX)){
    ...
    const prepareData = (data) => {
        ...
        const couponsToSend = coupons ? coupons.map(co => ({ "couponCode": co.label })) : undefined;
    ...
    return (
        <CustomModal translatedHeader={translate("affiliates.common.add-affiliate")} isOpen={isOpen} closeModal={closeModal}>
...
export default AddAffiliatesModal