import URLS from "../urls";
import ApiClient from "../api-client";

const generalUrl = URLS.SHIPMENT_PACKAGES.GENERAL;

const getPagination = async ({page = 1, limit = 10, search = "", locale = 'en'}) => {
    const res = await ApiClient.CustomAxios.get(generalUrl, {
        params: {
            page,
            limit,
            search,
        },
        headers: {
            'x-locale': locale
        }
    });

    return res.data;
};

const create = async ({name, description, dimensionUom, height, width, length, weightUom ,weight}) => {
    const dataToSend = {name, description, dimensionUom, height, width, length, weightUom ,weight};
    const res = await ApiClient.CustomAxios.post(generalUrl, dataToSend);
    return res.data;
};

const getById = async (id, {locale = 'en'}) => {
    const url = `${generalUrl}/${id}`;
    const res = await ApiClient.CustomAxios.get(url,
        {
            headers: {
                'x-locale': locale
            }
        });
    return res.data;
};

const update = async (id, {name, description, dimensionUom, height, width, length, weightUom ,weight}) => {
    const url = `${generalUrl}/${id}`;
    const dataToSend = {name, description, dimensionUom, height, width, length, weightUom ,weight};
    const res = await ApiClient.CustomAxios.post(url, dataToSend);
    return res.data;
};

const deleteById = async (id) => {
    const url = `${generalUrl}/${id}`;
    const res = await ApiClient.CustomAxios.delete(url);
    return res.data;
};

const ShipmentPackagesService = {
    getPagination,
    create,
    getById,
    update,
    deleteObject: deleteById,
};

export default ShipmentPackagesService;
