...
const generalUrl = URLS.TEAM_MEMBERS_URLS.GENERAL;
...
const create = async ({image, name, job, description}) => {
    ...
    formData.append("job[en]", job.en);
    formData.append("job[ar]", job.ar);
...
const update = async (id, {image, name, description, job}) => {
    const formData = new FormData();
    if (image) {
        formData.append("image", image[0]);
    }
